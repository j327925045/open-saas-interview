...
import { withAiCostControl } from "../ai-cost-control";
...
> = async (rawArgs, context) => {
  ...
  return withAiCostControl<GeneratedSchedule>(
    ...
    async () => {
      ...
      const inputTokens = estimateInputTokens(tasks, hours);
      const outputTokens = estimateOutputTokens(generatedSchedule);
...
function isUserSubscribed(user: { subscriptionStatus: string | null }): boolean {
  return (
...
function estimateInputTokens(tasks: Task[], hours: number): number {
  const tasksText = tasks.map((t) => t.description).join(" ");
  const promptBody = `I will work ${hours} hours today. Here are the tasks: ${tasksText}`;
  return Math.ceil(promptBody.length / 4) + 500;
}

function estimateOutputTokens(schedule: GeneratedSchedule): number {
